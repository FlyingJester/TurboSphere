...
typedef std::pair<const char *, TS_ConfigType> TS_ConfVal;
...
bool ExitGame(JSContext *ctx, unsigned argc, JS::Value *args){
    JS_DestroyContext(ctx);
    return false;
}

bool ChangeGame(JSContext *ctx, unsigned argc, JS::Value *vp){
    
    JS::CallArgs args = CallArgsFromVp(argc, vp);
    
    if(args.length()<1){
        JS::Value err_val(STRING_TO_JSVAL(JS_NewStringCopyZ(ctx, ENGINE " ChangeGame Error: Called with fewer than 1 argument.")));
        JS::Rooted<JS::Value> err(ctx, err_val);
        JS_SetPendingException(ctx, err);
        return false;
    }
    
    if(!args[0].isString()){
        JS::Value err_val(STRING_TO_JSVAL(JS_NewStringCopyZ(ctx, ENGINE " ChangeGame Error: Argument 0 is not a string.")));
        JS::Rooted<JS::Value> err(ctx, err_val);
        JS_SetPendingException(ctx, err);
        return false;
    }
    
    const char *str = JS_EncodeString(ctx, args[0].toString());
    
    if(!(t5::IsDir(str)||t5::IsFile(str))){
        JS::Value err_val(STRING_TO_JSVAL(JS_NewStringCopyZ(ctx, ENGINE " ChangeGame Error: Game does not exist.")));
        JS::Rooted<JS::Value> err(ctx, err_val);
        JS_SetPendingException(ctx, err);
        return false;
    }
    ...
    return ExitGame(ctx, argc, vp);
}

bool RestartGame(JSContext *ctx, unsigned argc, JS::Value *vp){
...
bool GetVersionNumber(JSContext *ctx, unsigned argc, JS::Value *vp){
    
    JS::Value version = JS_NumberValue(2.0);
...
bool GetVersionString(JSContext *ctx, unsigned argc, JS::Value *vp){
    JS::Value value(STRING_TO_JSVAL(JS_NewStringCopyZ(ctx, TS_GetVersion())));
    JS::Rooted<JS::Value> version(ctx, value);
